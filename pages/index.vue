<template>
    <PageTitle class="*:block *:md:flex">
            <div class="mx-10">
                <img src="https://raw.githubusercontent.com/FTCOpenAlliance/brandkit/main/svg/ICON.svg">
            </div>
            <div class="">
                <h1 class="my-5 text-6xl md:text-7xl xl:text-8xl font-bold text-orange">
                    FTC Open Alliance
                </h1>
                <h2 class="text-xl pb-5">
                    The place for open and collaborative robotics in FIRST Tech Challenge.
                </h2>
            </div>
    </PageTitle>
</template>

<script setup lang="ts">
useSeoMeta({
  title: 'FTC Open Alliance',
  ogTitle: 'FTC Open Alliance',
  description: 'The place for open and collaborative robotics in FIRST Tech Challenge',
  ogDescription: 'The place for open and collaborative robotics in FIRST Tech Challenge',
  ogImage: 'https://raw.githubusercontent.com/FTCOpenAlliance/brandkit/main/png_4k/bg/ICON.png',
})
</script>
<style>
    
</style>